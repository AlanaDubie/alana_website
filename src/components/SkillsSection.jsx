...
export const SkillsSection = () => {
    ...
    >
        <div className="container mx-auto max-w-8xl">
            <p className="eyebrow mb-6 text-left">
                Software
            </p>

            <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-2"> 
                {skills.map((skill, key) => (
                    <div key={key} className="bg-card p-2 rounded-sm border border-primary/10 card-hover hover:border-primary/30">
                        <div className="text-left m-1 flex items-center gap-2">
                            <img src={skill.icon} alt={`${skill.name} icon`} className="w-6 h-6" />
                            <h3 className="text-base font-medium text-foreground/90">{skill.name}</h3>
                        </div>
                    </div>
                ))}
            </div>
